...
func NewRouter(mockClient interface{}) *mux.Router {
	...
	v2r.HandleFunc(baseURL, h.handleTacIntentCreate).Methods("POST")
	v2r.HandleFunc(baseURL+"/{tac-intent}", h.handleTacIntentGet).Methods("GET")
	v2r.HandleFunc(baseURL, h.handleTacIntentGet).Methods("GET")
	v2r.HandleFunc(baseURL+"/{tac-intent}", h.handleTacIntentDelete).Methods("DELETE")
	v2r.HandleFunc(baseURL+"/{tac-intent}", h.handleTacIntentPut).Methods("PUT")
	// Cancel or get the status of a temporal action controller intent
	v2r.HandleFunc(baseURL+"/{tac-intent}/cancel", h.handleTemporalWorkflowHookCancel).Methods("POST")
	v2r.HandleFunc(baseURL+"/{tac-intent}/status", h.handleTemporalWorkflowHookStatus).Methods("GET")
	// Worker APIs - Used to register workers in DIGs so TAC can dynamically deploy and terminate them.
	v2r.HandleFunc(baseURL+"/{tac-intent}/workers", w.handleWorkerCreate).Methods("POST")
	v2r.HandleFunc(baseURL+"/{tac-intent}/workers", w.handleWorkerGet).Methods("GET")
	v2r.HandleFunc(baseURL+"/{tac-intent}/workers/{workers}", w.handleWorkerGet).Methods("GET")
	v2r.HandleFunc(baseURL+"/{tac-intent}/workers/{workers}", w.handleWorkerUpdate).Methods("PUT")
	v2r.HandleFunc(baseURL+"/{tac-intent}/workers/{workers}", w.handleWorkerDelete).Methods("DELETE")
