...
func getWorkers(tacIntents []model.WorkflowHookIntent, project, app, version, group string) ([]model.WorkerIntent, error) {
	logutils.Info("Getting all workers for relevant tac intents", logutils.Fields{})
...
func verifyWorkers(workers []model.WorkerIntent, project string) ([]model.WorkerIntent, error) {
	...
	for _, w := range workers {
		res, err := instantiationClient.Status(w.Spec.Project, w.Spec.CApp, w.Spec.CAppVersion, w.Spec.DIG, "", "", "", []string{}, []string{}, []string{})
		if err != nil {
			return []model.WorkerIntent{}, err
		}
		...
		if res.State.Actions[len(res.State.Actions)-1].State == "Approved" {
...
func deployWorkers(workers []model.WorkerIntent, project string) error {
	...
	for _, w := range workers {
		err := instantiationClient.Instantiate(w.Spec.Project, w.Spec.CApp, w.Spec.CAppVersion, w.Spec.DIG)
		if err != nil {
	...
	for i, w := range workers {
		...
		if res.State.Actions[len(res.State.Actions)-1].State == "Instantiated" {
...
func cleanUpWorkers(workers []model.WorkerIntent, project string) error {
	...
	// start all of the workers
	for _, w := range workers {
		err := instantiationClient.Terminate(w.Spec.Project, w.Spec.CApp, w.Spec.CAppVersion, w.Spec.DIG)
		if err != nil {
