...
// var (
// 	client = clusterprovider.NewClusterClient()
// 	mockdb *db.NewMockDB
// )
...
// 				cExists, err := client.CreateClusterGroup(mClusterGroup, "cert1", "provider1", true)
// 				validateError(err, "")
...
// 				l := len(mockdb.Items)
// 				err := client.DeleteClusterGroup("cert1", "cluster1", "provider1")
// 				validateError(err, "")
// 				Expect(len(mockdb.Items)).To(Equal(l - 1))
// 			})
// 		})
...
// 				cluster, err := client.GetClusterGroup("cert1", "cluster1", "provider1")
// 				validateError(err, "")
