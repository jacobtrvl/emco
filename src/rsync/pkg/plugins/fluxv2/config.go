...
func (p *Fluxv2Provider) ApplyConfig(ctx context.Context, config interface{}) error {
	...
	// might have to conver to the right files type
	// Commit
	err = emcogit.CommitFiles(ctx, p.gitProvider.Client, p.gitProvider.UserName, p.gitProvider.RepoName, p.gitProvider.Branch, "Commit for "+p.gitProvider.GetPath(), gp, p.gitProvider.GitType)
	if err != nil {
